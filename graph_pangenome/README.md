...
All scripts are run with `bash`; no `sbatch` wrapper is included.
...
- **Minigraph:** whole-genome construction.
...
Community/chromosome jobs are executed **sequentially** in this script to avoid multiplying peak memory usage.
...
VG is run on the whole reference genome and VCF; this workflow does **not** manually split chromosomes.
